...
export function EditionFormPanel({ raceId }: { raceId: string }) {
    ...
    <div className="rounded-xl border border-border bg-bg-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-ink">新增屆次</span>
        </div>
      ...
      {open && (
        <div className="px-5 pb-5 border-t border-border">
          <div className="pt-5">
            <EditionForm raceId={raceId} onSuccess={() => setOpen(false)} />
          </div>
        </div>
      )}
    </div>
